...
import static com.twoez.zupzup.history.domain.QPloggingLog.ploggingLog;
...
public class PloggingLogQueryRepository extends QuerydslRepositorySupport {
    ...
    public List<PloggingLog> findByBetweenStartDateAndEndDate(LocalDateTime startDate,
            LocalDateTime endDate, Long memberId) {
        return selectFrom(ploggingLog)
                .leftJoin(ploggingLog.member).fetchJoin()
    ...
    public List<PloggingLog> findByDate(LocalDate date, Long memberId) {
        return selectFrom(ploggingLog)
                ...
                .where(ploggingLog.member.id.eq(memberId)
                        ...
                        .and(ploggingLog.startDateTime.dayOfMonth().eq(date.getDayOfMonth())))
                .fetch();
    }

    public Optional<PloggingLog> findOneOrderByDateDesc(Long memberId) {
        return Optional.ofNullable(selectFrom(ploggingLog)
                .leftJoin(ploggingLog.member).fetchJoin()
                .where(ploggingLog.member.id.eq(memberId))
